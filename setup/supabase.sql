...
-- =========================================================
-- ANALÍTICA DE PLATAFORMAS (opcional pero recomendada)
-- Permite registrar accesos y consultas de bono sin guardar
-- nombres de usuario ni datos personales.
-- =========================================================
create table if not exists public.dz_platform_events (
  id bigint generated always as identity primary key,
  platform text not null check (platform in ('ganamos','bet30','trebol','deuna','apostamos')),
  event_type text not null check (event_type in ('open','bonus')),
  session_id uuid not null,
  created_at timestamptz not null default now()
);

create index if not exists dz_platform_events_lookup_idx
  on public.dz_platform_events(platform, event_type, created_at desc);

alter table public.dz_platform_events enable row level security;
revoke all on table public.dz_platform_events from anon, authenticated;

create or replace function public.dz_track_platform_event(p_platform text, p_event text, p_session uuid)
returns void
language plpgsql
security definer
set search_path = public, pg_temp
as $$
begin
  if p_platform not in ('ganamos','bet30','trebol','deuna','apostamos') then
    raise exception 'invalid platform';
  end if;
  if p_event not in ('open','bonus') then
    raise exception 'invalid event';
  end if;

  insert into public.dz_platform_events(platform, event_type, session_id)
  values (p_platform, p_event, p_session);
end;
$$;

revoke execute on function public.dz_track_platform_event(text,text,uuid) from public;
grant execute on function public.dz_track_platform_event(text,text,uuid) to anon, authenticated;
